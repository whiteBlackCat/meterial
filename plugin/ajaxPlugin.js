;(function(){
	...
	Ajax.init=function(params){
		new Ajax(params);//构造函数new后不赋值会发生什么  什么也不会发生（原本如此）但该ajax对象
	};                 //在实例化过程中调用start方法启动整个过程（这与平常面向对象很不同，init初始、检查
	                   //配置，启动主函数）   
	Ajax.prototype={
		constructor: Ajax,
		createXHR:function(){//创建ajax的兼容写法
		...
		start:function(params){
			xhr=new this.createXHR();//创建ajax对象
			//配置参数
			if(params.url){
